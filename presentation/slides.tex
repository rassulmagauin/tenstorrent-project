...
\begin{frame}{All three attempts, side by side}
  ...
  \begin{tabular}{lrr}
    ...
    \midrule
    (Failed) TopK vocab padding         & \alert{56.82} & \alert{$+36\%$} \\
    \bottomrule
  \end{tabular}
  \vfill
  \begin{itemize}
    \item Per-iteration decode: $10.4~\text{ms} \to 9.1~\text{ms}$
    \item Throughput (10 layers): $\sim$96 $\to$ $\sim$110 tok/s/user
    \item 2-layer smoke test: 65 $\to$ 102.7 tok/s/user (TopK fraction
          is larger at fewer layers, so the speedup looks bigger there)
